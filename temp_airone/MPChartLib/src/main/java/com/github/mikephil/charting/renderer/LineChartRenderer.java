    ...
    protected void drawLinear(Canvas c, ILineDataSet dataSet) {
        ...
        if (dataSet. getColors().size() > 1) {
            ...
            for (int j = mXBounds.min; j <= mXBounds.range + mXBounds.min; j++) {
                ...
                //TODO 그라데이션 추가.
                if(dataSet.getColors().size() >= (j+1)) {
                    if (isDrawSteppedEnabled) {
                        Shader shader = new LinearGradient(0, mLineBuffer[1], 0, mLineBuffer[7], dataSet.getColor(j), dataSet.getColor(j + 1), Shader.TileMode.MIRROR /*or REPEAT*/);
                        mRenderPaint.setShader(shader);
                    } else {
                        Shader shader = new LinearGradient(0, mLineBuffer[1], 0, mLineBuffer[3], dataSet.getColor(j), dataSet.getColor(j + 1), Shader.TileMode.MIRROR /*or REPEAT*/);
                        mRenderPaint.setShader(shader);
                    }
                }
